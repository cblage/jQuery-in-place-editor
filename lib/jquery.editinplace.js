...
InlineEditor.prototype = {
	...
	createSelectEditor: function () {
		...
		for (i = 0; i < optionsArray.length; i++) {
			...
			value = trim(currentTextAndValue[1] || currentTextAndValue[0]);
	...
	connectClosingEventsToEditor: function () {
		...
		} else if ('outsideClick' === this.settings.cancelOn) {
			$(document).bind('click.editInPlaceDeactivator', function (e) {
				if (that.openingEditor) {
					return;
				}
				var target = jQuery(e.target), activator;
								
				if (null !== that.settings.activator) {
					if ('parent' === that.settings.activatorType) {
						activator = that.dom.closest(that.settings.activator);
					} else {
						activator = $(that.settings.activator);
					}
					
					if (activator.length > 0 && (target.is(activator) || target.is(activator.children()))) {
						return;
					}
				}
				...
				if (target.is(form) || target.is(form.children())) {
					return;
				}				
	...
	{
		if ('outsideClick' === this.settings.cancelOn) {
			$(document).unbind('.editInPlaceDeactivator');
		}	
	},
	...
	handleSubmitToCallback: function (enteredText) {
		...
		var newHTML = this.triggerCallback(
			this.settings.callback, 
			enteredText, 
			this.originalValue, 
			this.settings.params, 
			this.savingAnimationCallbacks()
		);
		...
		$.ajax({
			...
			success: function (result) {			
				if (typeof that.settings.success === 'function' && 
				    that.triggerCallback(that.settings.success, result, reopenEditorCallback) !== false && 
				    that.settings.dataType === 'html'
				) {
					that.dom.html(result || that.settings.default_text);
				}
			},
			error: function (request) {
				if (typeof that.settings.error === 'function' && that.triggerCallback(that.settings.error, request, reopenEditorCallback) !== false) {
