...
InlineEditor.prototype = {
	...
	handleCancelEditor: function (anEvent, callDelegates) {
		if (typeof callDelegates === 'undefined') {
			callDelegates = true;
		}
		...
		if (callDelegates && false === this.triggerDelegateCall('shouldCloseEditInPlace', true, anEvent)) {
			return;
		}	
		
		if (callDelegates) {
			this.originalValue = this.triggerDelegateCall('willCloseEditInPlace', this.originalValue);
		}
		...
		$.ajax({
			...
			success: function (result) {			
				if (typeof that.settings.success === 'function' && 
				    that.triggerCallback(that.settings.success, result, reopenEditorCallback, cancelEditorCallback) !== false && 
			...
			error: function (request) {
				if (typeof that.settings.error === 'function' && that.triggerCallback(that.settings.error, request, reopenEditorCallback, cancelEditorCallback) !== false) {
					that.restoreOriginalValue();
